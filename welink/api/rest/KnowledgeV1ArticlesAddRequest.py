...
"""
创建知识
https://open.welink.huaweicloud.com/docs/#/990hh0/ka0vzc/fwsyfe?type=internal
...
class KnowledgeV1ArticlesAddRequest(RestApi):
    """创建知识"""

    def __init__(self, url):
        super(KnowledgeV1ArticlesAddRequest, self).__init__(url)
        self.__body_params = Article()
        self.__rest_method = "POST"
        self.__valid_path = ("/api/knowledge/v1/articles/add",)
    ...
    def cate_name(self):
        """
        string 文章所属分类，不传则取默认分类或第一个分类，传则取传过来的分类，没有则新建分类
        """
    ...
    def add_documents(self, value):
        """
        在请求实例中添加新的文章附件信息对象
        :param value: Document 需要添加的新的文章信息对象
        """
    ...
    def set_documents(self, value, index):
        """
        根据索引修改指定位置的文章附件信息对象
        :param value: Document 需要修改的新的文章附件信息对象
        :param index: int 索引
        """
